...
async function main(): Promise<void> {
  if (!process.argv.includes('--run')) {
    console.log(
      'SKIP: configure config.ini and run npm run test:thegamesdb:live -- --run to contact TheGamesDB.'
    );
    return;
  }
  const { providers } = await configuredProviders(
    new ConfigService(fileURLToPath(new URL('../', import.meta.url)))
  );
  const entry = providers.find((item) => item.provider.id === 'thegamesdb');
  if (!entry?.enabled || !entry.configured) {
    console.log(
      'SKIP: enable provider.thegamesdb with apiKey and add thegamesdb to metadata.providerOrder.'
    );
    return;
  }
  ...
  if (
    !details ||
    details.recordId !== candidate.recordId ||
    details.title !== candidate.title
  )
    throw new TheGamesDbError('live-detail-mismatch');
  console.log(
    'PASS: TheGamesDB authenticated search, details, and name normalization. No catalog or artwork files changed.'
  );
}
main().catch((error) => {
  console.error(
    error instanceof TheGamesDbError
      ? error.message
