...
async function main(): Promise<void> {
  if (!process.argv.includes('--run')) {
    console.log('SKIP: to contact IGDB, configure config.ini and run npm run test:igdb:live -- --run.');
    return;
  }
  ...
  if (!entry?.enabled || !entry.configured) {
    console.log('SKIP: add igdb to metadata.providerOrder and enable provider.igdb with clientId/clientSecret in config.ini.');
    return;
  }
  ...
  if (!details || details.recordId !== chosen.recordId || details.title !== chosen.title) throw new IgdbError('live-detail-mismatch');
  console.log('PASS: IGDB authentication, search, and normalized details. No catalog or artwork files were changed.');
}
main().catch(error => {
  console.error(error instanceof IgdbError ? error.message : 'Live check failed. Check local configuration and connectivity.');
  process.exitCode = 1;
});
