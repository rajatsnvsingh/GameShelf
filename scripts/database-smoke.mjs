...
try {
  ...
  try {
    ...
    const result = await application.evaluate((_electron, { entry, base }) => {
      ...
      const scan = {
        status: 'complete',
        collections: [{ folderName: 'Collection_Test', displayName: 'Test', relativePath: 'Collection_Test' }],
        games: [{ folderName: '日本語 Game', relativePath: 'Collection_Test/日本語 Game', collectionPath: 'Collection_Test' }]
      };
      ...
      try {
        ...
        repo.reconcile({ status: 'failed', error: { code: 'listing-failed', relativePath: 'Collection_Test' } });
    ...
    console.log('PASS: compiled repository and SQLite native binary in Electron; migrations, reconciliation, failed-scan preservation, and reopen.');
