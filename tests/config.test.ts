...
test('portable base uses the app location in development and launcher directory in a package', () => {
  assert.equal(
    portableBase(false, 'C:\\Apps\\GameShelf', 'Z:\\ignored'),
    'C:\\Apps\\GameShelf'
  );
  assert.equal(
    portableBase(true, 'C:\\Temp\\extracted', 'E:\\GameShelf'),
    'E:\\GameShelf'
  );
...
test('relative roots relocate across drive letters and preserve spaces and Unicode', () => {
  ...
  assert.equal(
    resolveRoot('G:\\GameShelf', relative),
    'G:\\GameShelf\\Jeux 日本語'
  );
...
test('release roots reject cross-drive, drive-qualified, UNC, app-containing and data-overlapping roots', () => {
  ...
  assert.equal(
    resolveRoot('E:\\GameShelf', 'database-games'),
    'E:\\GameShelf\\database-games'
  );
});
...
test('settings persist durable values while returning only redacted provider status', async (t) => {
  ...
  await service.saveSettings({
    ...
    showCollectionGames: false,
    matchingThreshold: 0.75,
    greedyMatch: true,
    defaultSort: 'releaseDate',
    providerOrder: ['thegamesdb', 'igdb'],
    providers: {
      igdb: { enabled: true, configured: false },
      thegamesdb: { enabled: true, configured: false },
      steamgriddb: { enabled: false, configured: false },
    },
    credentials: {
      igdbClientId: 'id',
      igdbClientSecret: 'secret',
      thegamesdbApiKey: 'key',
    },
  });
  const after = await service.getSettings();
  assert.equal(before.providers.igdb.configured, false);
  assert.equal(after.collectionPrefix, 'Set_');
  assert.equal(after.showCollectionGames, false);
  assert.equal(after.matchingThreshold, 0.75);
  assert.equal(after.greedyMatch, true);
  assert.deepEqual(after.providerOrder, ['thegamesdb', 'igdb']);
  assert.equal(after.providers.igdb.configured, true);
  ...
  assert.equal(stored['provider.igdb'].clientId, 'id');
  assert.equal(stored['provider.igdb'].clientSecret, 'secret');
  assert.equal(stored['provider.thegamesdb'].apiKey, 'key');
  await assert.rejects(() =>
    service.saveSettings({
      ...after,
      collectionPrefix: '../bad',
      providers: after.providers,
    })
  );
});
...
test('malformed INI and invalid settings are rejected without including values in errors', () => {
  for (const text of [
    ...
    assert.throws(
      () => withDefaults(parseIni(text)),
      (error) =>
        error instanceof Error && !error.message.includes('fake-secret')
    );
  }
});
...
async function fixture(t: { after: (fn: () => Promise<void>) => void }) {
  ...
  return {
    base,
    games,
    file: join(base, 'config.ini'),
    service: new ConfigService(base),
  };
}
...
test('missing config uses defaults, cancel writes nothing, selection persists a relative root', async (t) => {
  ...
  assert.equal(
    (await service.chooseRoot(async () => undefined)).status,
    'unconfigured'
  );
  ...
  assert.deepEqual(
    (await readdir(base)).sort(),
    ['Jeux 日本語', 'config.ini'].sort()
  );
});
...
test('invalid config is preserved and picker is not invoked', async (t) => {
  ...
  assert.equal(
    (
      await service.chooseRoot(async () => {
        picked = true;
        return undefined;
      })
    ).status,
...
test('selection rejects app/data roots, missing folders, files and junctions into data', async (t) => {
  ...
  for (const candidate of [
    ...
    assert.equal(
      (await service.chooseRoot(async () => candidate)).status,
      'unconfigured'
    );
    await assert.rejects(readFile(file), { code: 'ENOENT' });
  }
...
test('root updates preserve unknown future provider values and expose no credentials', async (t) => {
  ...
  assert.equal(
    parseIni(await readFile(file, 'utf8'))['provider.future'].key,
    'fake-secret'
  );
...
test('changing libraries preserves the existing catalog until an explicit rebuild', async (t) => {
  ...
  await mkdir(other);
  assert.match(
    (await service.chooseRoot(async () => other)).message,
    /Rebuild catalog/
  );
  assert.equal((await new ConfigService(base).getState()).root, other);
  assert.equal(
    (await service.chooseRoot(async () => games.toUpperCase())).status,
    'ready'
  );
});
...
test('overlapping picker requests open only one dialog', async (t) => {
  ...
  const first = service.chooseRoot(() => {
    release();
  ...
  await entered;
  await service.chooseRoot(async () => {
    assert.fail('Second picker must not open');
  });
  finish(undefined);
  await first;
});
