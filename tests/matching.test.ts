...
async function fixture(t: { after: (fn: () => Promise<void>) => void }) {
  ...
  const provider: MetadataProvider = {
    ...
    async search(query) {
      ...
      return [
        ...
        { recordId: '3', title: 'Game B Classic', releaseYear: 2024 },
      ];
    },
  ...
  const artworkOnlyProvider: MetadataProvider = {
    id: 'artwork-only',
    capabilities: { artwork: [], metadata: false },
    async search(query) {
      calls.push(`artwork:${query}`);
      return [{ recordId: 'art', title: query }];
    },
    async getGame(recordId) {
      return { recordId, title: 'Game A', artwork: [] };
    },
  };
  const artworkOnlyEntry = {
    provider: artworkOnlyProvider,
    enabled: false,
    configured: true,
  };
  ...
  const makeService = () =>
    new LibraryService(
      ...
      undefined,
      async () => ({
        providers: [entry, artworkOnlyEntry],
        threshold: 0.9,
        greedyMatch: false,
      })
    );
...
test('scan persists exact matches and retries every unresolved game without rematching saved bindings', async (t) => {
  ...
  assert.deepEqual(f.service.getMatchingStatus(), {
    phase: 'complete',
    attempted: 2,
    total: 2,
    matched: 1,
    gameName: 'Game B',
    message: '1 of 2 unresolved games matched.',
  });
...
test('manual search/selection persists binding, respects overrides, and survives rescan and restart', async (t) => {
  ...
  db.prepare('UPDATE games SET manual_overrides = ? WHERE id = ?').run(
    JSON.stringify({
      title: 'My title',
      description: null,
      cover: 'custom.png',
  ...
  const candidates = value(
    await f.service.searchMatches(game.id, 'Corrected title', 'fixture')
  ).candidates;
  assert.equal(candidates.length, 2);
  assert.equal(candidates[0].hasArtwork, true);
  assert.equal(
    value(await f.service.getCatalog()).games[1].matchStatus,
    'unmatched'
  );
  const saved = value(await f.service.selectMatch(game.id, 'fixture', '3'))
    .games[1];
  ...
  try {
    assert.deepEqual(value(await restarted.getCatalog()).games[1], scanned);
  } finally {
    restarted.close();
  }
  ...
  assert.deepEqual(
    JSON.parse(
      (
        check
          .prepare('SELECT manual_overrides FROM games WHERE id=?')
          .get(game.id) as { manual_overrides: string }
      ).manual_overrides
    ),
...
test('provider outages after local reconciliation preserve discoveries and continue matching later games', async (t) => {
  ...
  assert.ok(
    catalog.games.every(
      (game) => !game.missing && game.matchStatus === 'unmatched'
    )
  );
...
test('a provider failure for one game does not prevent a later game from matching', async (t) => {
  ...
  const catalog = value(await f.service.scan());
  assert.equal(
    catalog.games.find((game) => game.folderName === 'Game A')?.matchStatus,
    'unmatched'
  );
  assert.equal(
    catalog.games.find((game) => game.folderName === 'Game B')?.matchStatus,
    'matched'
  );
...
test('candidate selections require the latest search for that game and current settings', async (t) => {
  ...
  assert.equal(
    (await f.service.selectMatch(catalog.games[0].id, 'fixture', '3')).ok,
    false
  );
...
test('shutdown during retrieval prevents late match writes', async (t) => {
  ...
  try {
    assert.equal(
      value(await reopened.getCatalog()).games[1].matchStatus,
      'unmatched'
    );
  } finally {
    reopened.close();
...
test('metadata settings changed during detail retrieval discard the result', async (t) => {
  ...
  f.provider.getGame = async (recordId) => {
    ...
    await writeFile(
      path,
      (await readFile(path, 'utf8')).replace('0.75', '0.80')
    );
    return { recordId, title: 'Game B', artwork: [] };
  };
  assert.equal((await f.service.selectMatch(id, 'fixture', '3')).ok, false);
...
test('manual metadata and pasted artwork override provider data and survive restart', async (t) => {
  ...
  const overridden = value(
    await f.service.saveOverrides(game.id, {
      title: 'Personal title',
      description: null,
      genres: ['Custom'],
    })
  ).games[0];
  assert.equal(overridden.metadata.title, 'Personal title');
  assert.equal(overridden.metadata.description, undefined);
  assert.deepEqual(overridden.metadata.genres, ['Custom']);
  const png = new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]);
  const pasted = value(await f.service.pasteArtwork(game.id, 'cover', png))
    .games[0];
  assert.match(pasted.coverUrl!, /^gameshelf-artwork:\/\/local\/1-manual-/);
  assert.equal(
    (await f.service.pasteArtwork(game.id, 'cover', new Uint8Array([1]))).ok,
    false
  );
  f.service.close();
  const restarted = f.makeService();
  try {
    const saved = value(await restarted.getCatalog()).games[0];
    assert.equal(saved.metadata.title, 'Personal title');
    assert.match(saved.coverUrl!, /^gameshelf-artwork:/);
  } finally {
    restarted.close();
  }
});
...
test('replace-all rescrape clears manual work only after successful bound-provider retrieval', async (t) => {
  ...
  const game = value(await f.service.scan()).games[0];
  const png = new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]);
  value(await f.service.saveOverrides(game.id, { title: 'Keep me' }));
  value(await f.service.pasteArtwork(game.id, 'cover', png));
  f.provider.getGame = async () => {
    throw new Error('offline');
  };
  const failed = await f.service.replaceAllRescrape(game.id);
  assert.equal(failed.ok, false);
  assert.equal(
    value(await f.service.getCatalog()).games[0].metadata.title,
    'Keep me'
  );
  f.provider.getGame = async (recordId) => ({
    recordId,
    title: 'Game A',
    description: 'Fresh provider text',
    artwork: [],
  });
  const refreshed = value(await f.service.replaceAllRescrape(game.id)).games[0];
  assert.equal(refreshed.metadata.title, 'Game A');
  assert.equal(refreshed.metadata.description, 'Fresh provider text');
  assert.equal(refreshed.coverUrl, undefined);
});
...
test('maintenance removes only missing records and rebuild failure restores the original catalog', async (t) => {
  ...
  await rm(join(f.root, 'Game B'), { recursive: true, force: true });
  value(await f.service.scan());
  const removed = value(await f.service.deleteMissing());
  assert.equal(removed.games.length, 1);
  assert.equal(removed.games[0].id, first.games[0].id);
  const original = value(await f.service.getCatalog());
  await rm(f.root, { recursive: true, force: true });
  const failed = await f.service.rebuildCatalog();
  assert.equal(failed.ok, false);
...
test('wipe library clears catalog data but preserves configuration and installer folders', async (t) => {
  ...
  assert.ok(
    (await readFile(join(f.base, 'config.ini'), 'utf8')).includes('root')
  );
...
test('matching IPC rejects untrusted frames, arbitrary payloads, malformed IDs and oversized queries', () => {
  ...
  assert.deepEqual(validateSearchRequest(true, [1, 'Game A', 'igdb']), [
    1,
    'Game A',
    'igdb',
  ]);
  assert.deepEqual(validateSelectionRequest(true, [1, 'igdb', '123']), [
    1,
    'igdb',
    '123',
  ]);
  ...
  for (const args of [
    ...
    assert.throws(() => validateSearchRequest(true, args));
  for (const args of [
    [],
    [1, 'igdb'],
    [1, 'igdb', '1', {}],
    ...
    assert.throws(() => validateSelectionRequest(true, args));
  for (const args of [[], [1, '0'], [1, '-1'], [1, 1], [1, '1', 'extra']])
    assert.throws(() => validateArtworkRequest(true, args));
