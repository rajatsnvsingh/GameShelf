...
import { IgdbProvider, IgdbError, normalizeIgdbGame } from '../src/main/metadata/igdb.ts';
...
const settings = { enabled: true, clientId: 'fixture-client', clientSecret: 'fixture-secret' };
const token = { access_token: 'fixturetoken', token_type: 'bearer', expires_in: 3600 };
...
function fixture(responses: (Response | Error)[]) {
  ...
  const provider = new IgdbProvider(settings, { fetch, now: () => now, sleep: async ms => { now += ms; } });
...
test('IGDB obtains a token in a POST body, reuses it and normalizes search/details', async () => {
  ...
  assert.equal(new URLSearchParams(String(f.calls[0].init.body)).get('client_secret'), settings.clientSecret);
  assert.equal((f.calls[1].init.headers as Record<string, string>).Authorization, 'Bearer fixturetoken');
  for (const call of f.calls) { assert.equal(call.init.redirect, 'error'); assert.ok(!call.url.includes(settings.clientSecret)); }
...
test('IGDB disabled and unconfigured adapters never request tokens', async () => {
  for (const config of [{ ...settings, enabled: false }, { ...settings, clientSecret: '' }]) {
    let calls = 0;
    const provider = new IgdbProvider(config, { fetch: async () => { calls++; throw new Error(); } });
    await assert.rejects(provider.search('Game A'), code(config.enabled ? 'unconfigured' : 'disabled'));
    assert.equal(calls, 0);
  }
});

test('IGDB refreshes expired tokens and retries a rejected token only once', async () => {
  const f = fixture([json(token), json([game]), json(token), json([game]), json({}, 401), json(token), json({}, 401)]);
...
test('IGDB can recover after one expired-token rejection', async () => {
  const f = fixture([json(token), json({}, 401), json({ ...token, access_token: 'replacement' }), json([game])]);
  assert.equal((await f.provider.search('Game A'))[0].recordId, '1');
  assert.equal((f.calls[3].init.headers as Record<string, string>).Authorization, 'Bearer replacement');
});

test('IGDB rate limits impose cooldown without unbounded retry or waiting', async () => {
  const f = fixture([json(token), json({}, 429, { 'retry-after': '5' }), json([game])]);
...
test('IGDB request errors, authentication failures and malformed responses expose only safe codes', async () => {
  for (const [response, expected] of [
    [new Error('fixture-secret https://bad.invalid'), 'request-failed'], [json({ secret: 'fixture-secret' }, 403), 'forbidden'],
    [json({}, 500), 'http-error'], [new Response('not-json'), 'invalid-response'], [json({ access_token: 'leaked' }), 'invalid-token-response']
  ] as const) {
    const f = fixture([response]);
    await assert.rejects(f.provider.search('Game A'), error => {
      assert.ok(error instanceof IgdbError); assert.equal(error.code, expected);
      assert.ok(!String(error).includes('fixture-secret')); return true;
    });
  }
});
...
test('IGDB returns its bounded first search page while invalid records and mismatched details never become matches', async () => {
  ...
  await assert.rejects(mismatch.provider.getGame('1'), code('invalid-response'));
...
test('IGDB aborts a stalled request and refuses overlapping operations', async () => {
  let calls = 0;
  const provider = new IgdbProvider(settings, { timeoutMs: 20, sleep: async () => {}, fetch: async (_url, init) => {
    calls++;
    return new Promise((_resolve, reject) => init!.signal!.addEventListener('abort', () => reject(new Error('secret')), { once: true }));
  } });
...
test('IGDB integrates with the resolver while failures remain unresolved and existing bindings are preserved', async () => {
  ...
  const binding = { providerId: 'igdb', providerRecordId: '1', source: 'manual' as const, confidence: null };
  assert.deepEqual(await resolveMetadata('New name', [entry], { binding }), { status: 'preserved', binding });
  assert.equal(f.calls.length, 3);
  const failed = fixture([new Error('offline')]);
  assert.deepEqual(await resolveMetadata('Game A', [{ ...entry, provider: failed.provider }]),
    { status: 'unresolved', attempts: [{ providerId: 'igdb', outcome: 'error' }] });
});
