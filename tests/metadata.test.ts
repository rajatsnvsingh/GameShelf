...
import {
  resolveMetadata,
  titleConfidence,
} from '../src/main/metadata/resolver.ts';

function fake(
  id = 'fixture',
  candidates: readonly SearchCandidate[] = [{ recordId: '1', title: 'Game A' }]
) {
  const calls: string[] = [];
  const provider: MetadataProvider = {
    id,
    capabilities: { artwork: ['cover', 'background'] },
    async search(query) {
...
test('confidence preserves editions, punctuation and sequels; only case, whitespace and Unicode composition normalize', () => {
  ...
  for (const title of ['Game A 2', 'Game A Deluxe', 'Game-A', 'A Game']) {
    assert.ok(titleConfidence('Game A', title) < 0.9);
  }
...
test('selects highest confidence independent of provider result order and fetches only its details', async () => {
  for (const candidates of [
    [
      { recordId: '2', title: 'Game A Deluxe' },
      { recordId: '1', title: 'Game A' },
    ],
    [
      { recordId: '1', title: 'Game A' },
      { recordId: '2', title: 'Game A Deluxe' },
...
test('equal exact titles use the provider-ranked first record', async () => {
  const f = fake('fixture', [
    { recordId: 'old', title: 'Game A', releaseYear: 1999 },
    { recordId: 'new', title: 'Game A', releaseYear: 2024 },
  ]);
  const result = await resolveMetadata('Game A', [f.entry]);
  assert.equal(result.status, 'matched');
  if (result.status !== 'matched') assert.fail();
...
test('equal-title candidates prefer PC or Windows over a provider-ranked console entry', async () => {
  const f = fake('fixture', [
    { recordId: 'console', title: 'Game A', platforms: ['PlayStation 2'] },
    {
      recordId: 'pc',
      title: 'Game A',
  ...
  assert.equal(result.status, 'matched');
  if (result.status !== 'matched') assert.fail();
  assert.equal(result.binding.providerRecordId, 'pc');
  assert.deepEqual(f.calls, ['search:Game A', 'detail:pc']);
});
...
test('threshold is configurable and a close runner-up blocks matching even below threshold', async () => {
  ...
  assert.equal(
    (await resolveMetadata('Game A', [one.entry])).status,
    'matched'
  );
  assert.equal(
    (await resolveMetadata('Game A', [one.entry], { threshold: 0.9 })).status,
    'unresolved'
  );
  assert.equal(
    (await resolveMetadata('Game A', [one.entry], { threshold: 0.8 })).status,
    'matched'
  );
  const close = fake('fixture', [
    { recordId: '1', title: 'Game A Deluxe' },
    { recordId: '2', title: 'Game A Deluxe Edition' },
  ]);
  const result = await resolveMetadata('Game A', [close.entry], {
    threshold: 0.8,
  });
  // 0.80 versus 0.667 is separated enough; two 0.80 candidates are not.
  assert.equal(result.status, 'matched');
  const near = fake('fixture', [
    { recordId: '1', title: 'One Two Three Four Extra' },
    { recordId: '2', title: 'One Two Three Four Extra Edition' },
  ]);
  assert.deepEqual(
    await resolveMetadata('One Two Three Four', [near.entry], {
      threshold: 0.88,
    }),
    {
      status: 'unresolved',
      attempts: [{ providerId: 'fixture', outcome: 'ambiguous' }],
    }
  );
});
...
test('greedy matching accepts the first provider-ranked result without threshold or ambiguity checks', async () => {
  const f = fake('fixture', [
    { recordId: 'first', title: 'Unrelated' },
    { recordId: 'exact', title: 'Game A' },
  ]);
  const result = await resolveMetadata('Game A', [f.entry], {
    greedyMatch: true,
  });
...
test('zero confidence never matches even with a zero threshold; blank input makes no requests', async () => {
  const f = fake();
  assert.equal(
    (await resolveMetadata('Unrelated', [f.entry], { threshold: 0 })).status,
    'unresolved'
  );
  f.calls.length = 0;
  assert.deepEqual(await resolveMetadata('  ', [f.entry]), {
    status: 'unresolved',
    attempts: [],
  });
...
test('disabled and unconfigured providers are never called; no providers is valid', async () => {
  ...
  assert.deepEqual(
    await resolveMetadata('Game A', [disabled.entry, unconfigured.entry]),
    {
      status: 'unresolved',
      attempts: [
        { providerId: 'disabled', outcome: 'disabled' },
        { providerId: 'unconfigured', outcome: 'unconfigured' },
      ],
    }
  );
  assert.deepEqual([...disabled.calls, ...unconfigured.calls], []);
  assert.deepEqual(await resolveMetadata('Game A', []), {
    status: 'unresolved',
    attempts: [],
  });
});
...
test('uses supplied priority and stops after the provider-ranked exact match', async () => {
  ...
  const ambiguous = fake('ambiguous', [
    { recordId: '1', title: 'Game A' },
    { recordId: '2', title: 'Game A' },
  ]);
  const chosen = fake('chosen');
  const later = fake('later');
  const result = await resolveMetadata('Game A', [
    empty.entry,
    ambiguous.entry,
    chosen.entry,
    later.entry,
...
test('search and detail errors stay distinct from no match and hide raw exception secrets', async () => {
  for (const method of ['search', 'getGame'] as const) {
    ...
    broken.provider[method] = async () => {
      throw new Error('fixture-secret-in-request-url');
    };
    ...
    assert.deepEqual(failed, {
      status: 'unresolved',
      attempts: [{ providerId: 'broken', outcome: 'error' }],
    });
  }
});
...
test('missing details and mismatched detail IDs/titles do not bind', async () => {
  for (const details of [
    null,
    { recordId: 'wrong', title: 'Game A', artwork: [] },
    { recordId: '1', title: 'Game A 2', artwork: [] },
  ]) {
    const f = fake();
...
test('duplicate records are deduplicated while conflicting duplicates fail safely', async () => {
  ...
  assert.equal(
    (await resolveMetadata('Game A', [f.entry, f.entry])).status,
    'matched'
  );
  assert.deepEqual(f.calls, ['search:Game A', 'detail:1']);
  const conflict = fake('fixture', [
    duplicate,
    { recordId: '1', title: 'Other' },
  ]);
  assert.deepEqual(await resolveMetadata('Game A', [conflict.entry]), {
    status: 'unresolved',
    attempts: [{ providerId: 'fixture', outcome: 'error' }],
  });
});
...
test('malformed normalized search responses fail without fetching details', async () => {
  for (const invalid of [
    null,
    [{ title: 'Game A' }],
    [{ recordId: '', title: 'Game A' }],
    [null],
  ]) {
    const f = fake();
    f.provider.search = async () => invalid as unknown as SearchCandidate[];
    assert.deepEqual(await resolveMetadata('Game A', [f.entry]), {
      status: 'unresolved',
      attempts: [{ providerId: 'fixture', outcome: 'error' }],
    });
    assert.deepEqual(f.calls, []);
...
test('manual and automatic bindings survive changed priorities, names, disabled providers, and offline operation', async () => {
  for (const source of ['manual', 'automatic'] as const) {
    const binding: ProviderBinding = Object.freeze({
      providerId: 'original',
      providerRecordId: 'fixed-id',
      source,
      confidence: source === 'manual' ? null : 1,
    });
    ...
    assert.deepEqual(
      await resolveMetadata('Different folder name', [competing.entry], {
        binding,
      }),
      { status: 'preserved', binding }
    );
    assert.deepEqual(await resolveMetadata('Game A', [], { binding }), {
      status: 'preserved',
      binding,
    });
...
test('normalized detail/artwork data stays in the proposal with missing fields omitted', async () => {
  ...
  const details: GameDetails = {
    ...
    rating: 85,
    artwork: [{ kind: 'cover', url: 'https://fixture.invalid/cover.jpg' }],
  };
  f.provider.getGame = async () => details;
  const result = await resolveMetadata('Game A', [f.entry]);
  if (result.status !== 'matched') assert.fail();
...
test('invalid thresholds fail before providers are invoked', async () => {
  ...
  for (const threshold of [-1, 1.1, NaN, Infinity]) {
    await assert.rejects(
      resolveMetadata('Game A', [f.entry], { threshold }),
