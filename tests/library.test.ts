...
test('startup and root selection do not scan or create a catalog', async (t) => {
  ...
  const { base, root, service } = await fixture(t, async (path) => {
    calls++;
    return createScanReader(path);
  });
  assert.deepEqual(value(await service.getCatalog()), {
    games: [],
    collections: [],
  });
  ...
  await assert.rejects(readFile(join(base, 'data', 'library.db')), {
    code: 'ENOENT',
  });
});
...
test('manual scan uses real shallow listings, persists membership, and opens the collection game by ID', async (t) => {
  ...
  const { base, root, service, opened } = await fixture(t, async (path) => {
    ...
    return {
      ...reader,
      listDirectory: (requestRoot, relative) => {
        visits.push(relative);
  ...
  assert.equal(
    member.collectionId,
    catalog.collections.find((collection) => collection.displayName === 'One')!
      .id
  );
  ...
  assert.equal(
    await readFile(join(root, 'Game A', 'sentinel.txt'), 'utf8'),
    'untouched fixture'
  );
...
test('changing the selected library requires and then permits an explicit rebuild', async (t) => {
  ...
  if (!scan.ok)
    assert.match(scan.message, /Open Settings and choose Rebuild catalog/);
  const rebuilt = value(await service.rebuildCatalog());
  assert.deepEqual(
    rebuilt.games.map((game) => game.folderName),
    ['Only Here']
  );
});
...
test('scan recognizes immediate archives, skips underscore folders, and opens archive-specific locations', async (t) => {
  ...
  assert.equal(
    catalog.games.some(
      (game) =>
        game.folderName === '_Private' || game.folderName === '_Hidden Member'
    ),
    false
  );
  const zip = catalog.games.find((game) => game.relativePath === 'Root.zip')!;
  const rar = catalog.games.find(
    (game) => game.relativePath === '[C] One/Collection.RAR'
  )!;
  const iso = catalog.games.find((game) => game.relativePath === 'Disc.iso')!;
  assert.ok(zip && rar && iso);
  assert.deepEqual(
    [zip.folderName, rar.folderName, iso.folderName],
    ['Root', 'Collection', 'Disc']
  );
  assert.deepEqual(
    [zip.displayName, rar.displayName, iso.displayName],
    ['Root', 'Collection', 'Disc']
  );
  ...
  assert.deepEqual(opened, [
    join(root, 'Root.zip'),
    join(root, '[C] One', 'Collection.RAR'),
    await realpath(root),
  ]);
});
...
test('missing folders fail opening before a rescan and reappear with the same catalog identity', async (t) => {
  ...
  await service.chooseRoot(async () => root);
  const original = value(await service.scan()).games.find(
    (game) => game.folderName === 'Game B'
  )!;
  ...
  assert.deepEqual(opened, []);
  assert.equal(
    value(await service.getCatalog()).games.find(
      (game) => game.id === original.id
    )!.missing,
    false
  );
  assert.equal(
    value(await service.scan()).games.find((game) => game.id === original.id)!
      .missing,
    true
  );
  await rename(join(base, 'Absent'), folder);
  const returned = value(await service.scan()).games.find(
    (game) => game.id === original.id
  )!;
...
test('unavailable root preserves cached browsing across restart and cannot mark games missing', async (t) => {
  ...
  try {
    assert.deepEqual(value(await restarted.getCatalog()), before);
  } finally {
    restarted.close();
  }
});
...
test('a collection-listing failure after discoveries leaves all persisted records unchanged', async (t) => {
  ...
  const { root, service } = await fixture(t, async (path) => {
    ...
    return {
      ...reader,
      async listDirectory(requestRoot, relative) {
        if (fail && relative === '[C] One')
...
test('folder resolution rejects traversal, files, deeper paths, and changed collection junctions', async (t) => {
  const { base, root, service, opened } = await fixture(t);
  await service.chooseRoot(async () => root);
  const member = value(await service.scan()).games.find(
    (game) => game.folderName === 'Game B'
  )!;
  for (const path of [
    '../Outside',
...
test('unknown or invalid IDs never invoke Explorer; OS errors are reported without raw details', async (t) => {
  const { root, service, opened } = await fixture(
    t,
    createScanReader,
    'raw OS detail'
  );
  await service.chooseRoot(async () => root);
  const catalog = value(await service.scan());
  for (const id of [-1, 0, 1.5, NaN, 9999])
    assert.equal((await service.openInstallFolder(id)).ok, false);
  assert.deepEqual(opened, []);
