...
import {
  ...
  type DirectoryEntry,
  type ListDirectory,
} from '../src/main/library/scanner.ts';
...
function listing(tree: Record<string, readonly DirectoryEntry[]>) {
  ...
  const list: ListDirectory = async (root, relative) => {
    ...
    calls.push(relative);
    if (!Object.hasOwn(tree, relative))
      throw new Error('Unexpected directory visit');
...
test('discovers root games and collection members with literal names and full relative paths', async () => {
  const { list, calls } = listing({
    '': [
      directory('Z Game [Edition]'),
      directory('Collection_Favorites'),
      directory('A Game'),
    ],
  ...
  assert.deepEqual(await scanLibrary(input, list), {
    ...
    games: [
      ...
      {
        folderName: 'A Game',
        relativePath: 'Collection_Favorites/A Game',
        collectionPath: 'Collection_Favorites',
      },
      {
        folderName: '日本語 Game',
        relativePath: 'Collection_Favorites/日本語 Game',
        collectionPath: 'Collection_Favorites',
      },
      {
        folderName: 'Z Game [Edition]',
        relativePath: 'Z Game [Edition]',
        collectionPath: null,
      },
    ],
    collections: [
      {
        folderName: 'Collection_Favorites',
        displayName: 'Favorites',
        relativePath: 'Collection_Favorites',
      },
    ],
  });
...
test('empty library succeeds; empty game folders count without being listed', async () => {
  assert.deepEqual(await scanLibrary(input, listing({ '': [] }).list), {
    status: 'complete',
    games: [],
    collections: [],
  });
...
test('empty collections remain collections, including a prefix-only folder', async () => {
  ...
  assert.deepEqual(result, {
    ...
    collections: [
      {
        folderName: 'Collection_',
        displayName: '',
        relativePath: 'Collection_',
      },
      {
        folderName: 'Collection_Empty',
        displayName: 'Empty',
        relativePath: 'Collection_Empty',
      },
    ],
  });
});
...
test('only supported immediate single files imply games', async () => {
  ...
  if (result.status === 'complete')
    assert.deepEqual(result.games, [
      {
        folderName: 'Game.zip',
        relativePath: 'Collection_Files/Game.zip',
        collectionPath: 'Collection_Files',
      },
      {
        folderName: 'setup.exe',
        relativePath: 'Collection_Files/setup.exe',
        collectionPath: 'Collection_Files',
      },
      {
        folderName: 'Game.iso',
...
test('discovers supported archives only at the root or directly in [C] collections', async () => {
  ...
  const { list, calls } = listing({
    '': [
      file('Root.ZIP'),
      file('Unsupported.7z'),
      directory('[C] Classics'),
      directory('_Ignored'),
      directory('Folder Game'),
    ],
    '[C] Classics': [
      file('Collection.rar'),
      file('Disc.iso'),
      file('setup.exe'),
      directory('_Hidden Game'),
      directory('Folder Member'),
    ],
  });
  ...
  assert.deepEqual(result, {
    ...
    games: [
      {
        folderName: 'Folder Game',
        relativePath: 'Folder Game',
        collectionPath: null,
      },
      {
        folderName: 'Root.ZIP',
        relativePath: 'Root.ZIP',
        collectionPath: null,
      },
      {
        folderName: 'Collection.rar',
        relativePath: '[C] Classics/Collection.rar',
        collectionPath: '[C] Classics',
      },
      {
        folderName: 'Disc.iso',
        relativePath: '[C] Classics/Disc.iso',
        collectionPath: '[C] Classics',
      },
      {
        folderName: 'Folder Member',
        relativePath: '[C] Classics/Folder Member',
        collectionPath: '[C] Classics',
      },
      {
        folderName: 'setup.exe',
        relativePath: '[C] Classics/setup.exe',
        collectionPath: '[C] Classics',
      },
    ],
    collections: [
      {
        folderName: '[C] Classics',
        displayName: 'Classics',
        relativePath: '[C] Classics',
      },
    ],
  });
...
test('never visits game contents or treats collection members as nested collections', async () => {
  ...
  if (result.status === 'complete') {
    ...
    assert.deepEqual(
      result.games.map((game) => game.relativePath),
      ['Collection_Outer/Collection_Inner', 'Collection_Outer/Member', 'Game']
    );
  }
...
test('skips links/junctions and special entries at both levels without following targets', async () => {
  const { list, calls } = listing({
    '': [
      { name: 'Collection_Link', kind: 'link' },
      { name: 'Game Link', kind: 'link' },
      { name: 'Special', kind: 'other' },
      directory('Collection_Real'),
    ],
    Collection_Real: [
      { name: 'Outside Root', kind: 'link' },
      { name: 'Special', kind: 'other' },
    ],
  });
  const result = await scanLibrary(input, list);
...
test('custom prefix is matched exactly and only at the root', async () => {
  const { list, calls } = listing({
    '': [
      directory('Set_One'),
      directory('set_lowercase'),
      directory('Collection_Default'),
    ],
    Set_One: [directory('Set_Child')],
  });
  const result = await scanLibrary(
    { ...input, collectionPrefix: 'Set_' },
    list
  );
  assert.equal(result.status, 'complete');
  if (result.status === 'complete') {
    assert.deepEqual(
      result.collections.map((collection) => collection.displayName),
      ['One']
    );
...
test('listing order does not affect output or collection visit order; inputs are not mutated', async () => {
  const tree = {
    '': Object.freeze([
      directory('z'),
      directory('Collection_Z'),
      directory('A'),
      directory('Collection_A'),
    ]),
  ...
  const reversed = listing(
    Object.fromEntries(
      Object.entries(tree).map(([key, entries]) => [
        key,
        [...entries].reverse(),
      ])
    )
  ...
  assert.deepEqual(
    await scanLibrary(input, forward.list),
    await scanLibrary(input, reversed.list)
  );
...
test('root listing failure is distinguishable from an empty library and hides raw errors', async () => {
  assert.deepEqual(
    await scanLibrary(input, async () => {
      throw new Error('sensitive filesystem detail');
    }),
    {
      status: 'failed',
      error: { code: 'listing-failed', relativePath: '' },
    }
  );
});
...
test('collection failure discards earlier discoveries and stops further listing', async () => {
  ...
  assert.deepEqual(result, {
    status: 'failed',
    error: { code: 'listing-failed', relativePath: 'Collection_B' },
  });
...
test('unsafe adapter names fail without visiting paths outside the scan grammar', async () => {
  ...
  assert.deepEqual(
    await scanLibrary(
      ...
      listing({
        '': [directory('Collection_A')],
        Collection_A: [directory('../escape')],
      }).list
    ),
    {
      status: 'failed',
      error: { code: 'invalid-entry', relativePath: 'Collection_A' },
    }
  );
});
...
test('invalid root or prefix fails before calling the adapter', async () => {
  let called = false;
  const list: ListDirectory = async () => {
    called = true;
    return [];
  };
  for (const invalid of [
    ...
    assert.deepEqual(await scanLibrary(invalid, list), {
      status: 'failed',
