...
import type { GameDetails, MetadataProvider } from '../src/main/metadata/provider.ts';
import type { OperationResult } from '../src/shared/api.ts';

const url = (id: number) => `https://images.igdb.com/igdb/image/upload/t_1080p/shot${id}.jpg`;
const jpeg = new Uint8Array([0xff, 0xd8, 0xff, 0x00]);
const scan = { status: 'complete' as const, collections: [], games: [{ folderName: 'Game', relativePath: 'Game', collectionPath: null }] };
...
test('IGDB screenshots are separate, deduplicated, validated and capped at five', () => {
  const details = normalizeIgdbGame({ id: 1, name: 'Game', screenshots: [null, { image_id: '../bad' },
    { image_id: 'shot1', url: 'https://evil.invalid' }, ...Array.from({ length: 8 }, (_, i) => ({ image_id: `shot${i + 1}` }))] });
  assert.deepEqual(details.screenshots, Array.from({ length: 5 }, (_, i) => ({ url: url(i + 1) })));
  assert.deepEqual(details.artwork, []);
  assert.deepEqual(normalizeIgdbGame({ id: 1, name: 'Game' }).screenshots, []);
  assert.equal(normalizeIgdbGame({ id: 1, name: 'Game', screenshots: 'invalid' }).screenshots, undefined);
});
...
test('v2 catalogs upgrade with empty galleries and enforce binding, limits and cascade deletion', () => {
  ...
  try {
    ...
    repo.reconcile(scan);
    repo.saveMatch(1, { providerId: 'igdb', providerRecordId: '1', source: 'manual', confidence: null }, { recordId: '1', title: 'Game', artwork: [] });
    ...
    assert.throws(() => repo.replaceScreenshots(1, '1', Array(6).fill(items[0])));
    assert.throws(() => repo.replaceScreenshots(1, '1', [{ ...items[0], localPath: '../escape.jpg' }]));
    assert.throws(() => repo.replaceScreenshots(1, '2', items));
    assert.deepEqual(repo.listScreenshots(1), items);
    assert.deepEqual(repo.listGames()[0].manualOverrides, { title: 'My title' });
    repo.saveMatch(1, { providerId: 'igdb', providerRecordId: '2', source: 'manual', confidence: null }, { recordId: '2', title: 'Other', artwork: [] });
    assert.deepEqual(repo.listScreenshots(1), [], 'old match screenshots must never show for another game');
...
test('screenshots cache on matching, survive failures/offline restart, and fetch without replacing manual work', async t => {
  ...
  let details: GameDetails = { recordId: '1', title: 'Game', artwork: [], screenshots: Array.from({ length: 7 }, (_, i) => ({ url: url(i + 1) })) };
  const provider: MetadataProvider = { id: 'igdb', capabilities: { artwork: [] },
    async search() { return [{ recordId: '1', title: 'Game' }]; },
    async getGame() { if (offline) throw new Error('offline'); return details; } };
  ...
  const make = () => new LibraryService(base, config, async () => '', undefined,
    async () => ({ providers: [entry], threshold: .75, greedyMatch: false }), cache);
  let service = make();
  ...
  for (const local of game.screenshotUrls!) assert.deepEqual(await readFile(join(base, 'data', 'artwork', new URL(local).pathname.slice(1))), Buffer.from(jpeg));
  value(await service.saveOverrides(game.id, { title: 'My title' }));
  value(await service.pasteArtwork(game.id, 'cover', new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10])));
  downloads.length = 0;
  value(await service.scan());
  const before = value(await service.getCatalog()).games[0];
  assert.equal(downloads.length, 0, 'normal scans preserve existing bindings and gallery');
  offline = true;
  assert.equal((await service.fetchScreenshots(game.id)).ok, false);
  service.close(); service = make();
  ...
  assert.deepEqual(value(await service.fetchScreenshots(game.id)).games[0].screenshotUrls, []);
...
test('screenshot cache rejects unapproved origins and invalid image bytes', async t => {
  ...
  let calls = 0;
  const cache = new ArtworkCache(base, async () => { calls++; return new Response('not an image'); });
  await assert.rejects(cache.download(1, { kind: 'screenshot', url: 'https://evil.invalid/shot.jpg' }));
...
test('bulk missing-metadata fetch preserves bindings and manual work while continuing after provider failures', async t => {
  ...
  const provider: MetadataProvider = {
    id: 'igdb', capabilities: { artwork: [] },
    async search(query) { return [{ recordId: query === 'Game A' ? '1' : '2', title: query }]; },
    async getGame(recordId) {
      calls.push(recordId);
      if (recordId === '2' && failB) throw new Error('offline');
      return { recordId, title: recordId === '1' ? 'Game A' : 'Game B', description: `Provider ${recordId}`,
        artwork: [], screenshots: [{ url: url(Number(recordId)) }] };
    }
  };
  const entry = { provider, enabled: true, configured: true };
  const config = new ConfigService(base);
  const cache = new ArtworkCache(base, async () => new Response(jpeg));
  const service = new LibraryService(base, config, async () => '', undefined,
    async () => ({ providers: [entry], threshold: .75, greedyMatch: false }), cache);
  t.after(async () => { service.close(); await rm(base, { recursive: true, force: true }); });
  await service.chooseRoot(async () => join(base, 'Games'));
  ...
  value(await service.pasteArtwork(a.id, 'cover', new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10])));
  ...
  assert.match(result.message!, /Fetched missing data for 1 of 2 matched games/);
