...
test('artwork cache uses opaque relative names and leaves no file after an interrupted response', async (t) => {
  ...
  const name = await cache.download(7, {
    kind: 'cover',
    url: 'https://images.example/secret-name.jpg',
  });
  assert.match(name, /^7-cover-[a-f0-9]{24}\.jpg$/);
  assert.deepEqual(
    await readFile(join(base, 'data', 'artwork', name)),
    Buffer.from(jpeg)
  );
  const broken = new ArtworkCache(base, async () => {
...
test('provider artwork cannot replace a future manual asset', async (t) => {
  const base = await mkdtemp(join(tmpdir(), 'gameshelf-artwork-repo-'));
  ...
  db.prepare(
    "INSERT INTO artwork VALUES (?, 'cover', 'manual', 'manual.png', NULL)"
  ).run(game.id);
  assert.equal(
  ...
  assert.deepEqual(repo.listArtwork(game.id), [
    { kind: 'cover', source: 'manual', localPath: 'manual.png' },
  ]);
});

test('a provider artwork refresh replaces an existing provider cache row', async (t) => {
  const base = await mkdtemp(join(tmpdir(), 'gameshelf-artwork-refresh-'));
  const repo = openCatalog(base, 'Games');
  t.after(async () => {
    repo.close();
    await rm(base, { recursive: true, force: true });
  ...
  assert.deepEqual(repo.listArtwork(game.id), [
    { kind: 'cover', source: 'provider', localPath: 'replacement.jpg' },
  ]);
});

test('a confirmed provider replacement can replace user-supplied artwork', async (t) => {
  const base = await mkdtemp(join(tmpdir(), 'gameshelf-artwork-confirm-'));
  const repo = openCatalog(base, 'Games');
  t.after(async () => {
    repo.close();
    await rm(base, { recursive: true, force: true });
  ...
  assert.equal(
    repo.replaceWithProviderArtwork(
      ...
      'cover',
      'confirmed.jpg',
      'https://images.example/confirmed.jpg'
    ),
    true
  );
  assert.deepEqual(repo.listArtwork(game.id), [
    { kind: 'cover', source: 'provider', localPath: 'confirmed.jpg' },
  ]);
});
...
test('SteamGridDB is artwork-only and sends its key only in an authorization header', async () => {
  ...
  const provider = new SteamGridDbProvider(
    ...
    async (_url, init) => {
      calls.push(init!);
      return new Response(
        JSON.stringify({ success: true, data: [{ id: 4, name: 'Game' }] })
      );
    }
  );
  assert.deepEqual(await provider.search('Game'), [
    { recordId: '4', title: 'Game' },
  ]);
