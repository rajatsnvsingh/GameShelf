...
function snapshot(
  paths: string[] = ['Game A', 'Collection_One/Game B'],
  collectionNames = ['Collection_One']
): ScanResult {
  return {
    status: 'complete',
    collections: collectionNames.map((name) => ({
      folderName: name,
      displayName: name.slice('Collection_'.length),
      relativePath: name,
    })),
    games: paths.map((path) => {
      ...
      return {
        ...
        relativePath: path,
        collectionPath: parts.length === 1 ? null : parts[0],
      };
    }),
  };
}
...
test('catalog identity normalizes separators and case while preserving literal path spelling', () => {
  assert.deepEqual(catalogPath('Collection_One\\日本語 Game'), {
    path: 'Collection_One/日本語 Game',
    key: 'collection_one/日本語 game',
    parts: ['Collection_One', '日本語 Game'],
  });
  for (const path of [
    '',
    '.',
...
test('failed migration batch rolls back schema and history while preserving existing data', (t) => {
  ...
  assert.equal(
    db
      .prepare(
        "SELECT name FROM sqlite_master WHERE name = 'fixture_extension'"
      )
      .get(),
    undefined
  );
...
test('future, inconsistent, or nonconsecutive migrations are refused', (t) => {
  const { db } = memory(t);
  assert.throws(
    () => applyMigrations(db, [{ version: 2, name: 'gap', sql: '' }]),
    /consecutive/
  );
...
test('complete scans add collections and games, with valid membership and unmatched defaults', (t) => {
  ...
  assert.equal(
    games.find((game) => game.folderName === 'Game B')!.collectionId,
    collections[0].id
  );
  assert.equal(
    games.find((game) => game.folderName === 'Game A')!.collectionId,
    null
  );
...
test('repeat scans preserve IDs and added dates, update last seen, and do not duplicate records', (t) => {
  ...
  assert.deepEqual(
    repo.listGames(),
    beforeGames.map((game) => ({ ...game, lastSeenAt: later }))
  );
  assert.deepEqual(
    repo.listCollections(),
    beforeCollections.map((collection) => ({
      ...collection,
      lastSeenAt: later,
    }))
  );
...
test('absent games and collections become missing without deletion and reappear with the same IDs', (t) => {
  ...
  const missing = repo
    .listGames()
    .find((game) => game.folderName === 'Game B')!;
  ...
  assert.deepEqual(
    repo.listGames(),
    beforeGames.map((game) => ({ ...game, lastSeenAt: last }))
  );
  assert.deepEqual(
    repo.listCollections(),
    beforeCollections.map((collection) => ({ ...collection, lastSeenAt: last }))
  );
});
...
test('scans preserve provider bindings, metadata, overrides and explicit manual clearing', (t) => {
  ...
  repo.reconcile(snapshot(), first);
  db.prepare(
    `UPDATE games SET match_status = 'matched', binding_source = 'manual', provider_id = ?,
    provider_record_id = ?, confidence = ?, provider_metadata = ?, manual_overrides = ?`
  ).run(
    'fixture-provider',
    'fixture-record',
    0.97,
  ...
  assert.deepEqual(
    repo.listGames(),
    before.map((game) => ({ ...game, lastSeenAt: last }))
  );
});
...
test('case/separator changes preserve identity; moves and renames create new entries', (t) => {
  ...
  const original = repo
    ...
    .find((game) => game.folderName === 'Game B')!;
  repo.reconcile(
    snapshot(['GAME A', 'collection_one\\GAME B'], ['collection_one']),
    later
  );
  assert.equal(
    repo.listGames().find((game) => game.folderName === 'GAME B')!.id,
    original.id
  );
  assert.equal(
    repo.listGames().find((game) => game.id === original.id)!.relativePath,
    'collection_one/GAME B'
  );
  repo.reconcile(snapshot(['Game B', 'Renamed A'], []), last);
  assert.equal(repo.listGames().length, 4);
  assert.equal(
    repo.listGames().find((game) => game.id === original.id)!.missing,
    true
  );
  assert.notEqual(
    repo.listGames().find((game) => game.relativePath === 'Game B')!.id,
    original.id
  );
});
...
test('same folder names in different collections remain distinct', (t) => {
  ...
  repo.reconcile(
    snapshot(
      ['Collection_A/Game', 'Collection_B/Game'],
...
test('failed root or collection scans make no changes to any catalog record', async (t) => {
  ...
  for (const failAt of ['', 'Collection_One']) {
    const result = await scanLibrary(
      { root: 'E:\\Games', collectionPrefix: 'Collection_' },
      async (_root, relative) => {
        if (relative === failAt) throw new Error('unavailable');
        return [
          { name: 'New Game', kind: 'directory' },
          { name: 'Collection_One', kind: 'directory' },
        ];
      }
    );
    assert.equal(repo.reconcile(result, later), 'skipped');
    assert.deepEqual(
      { games: repo.listGames(), collections: repo.listCollections() },
      before
    );
  }
});
...
test('malformed complete snapshots fail before changing presence', (t) => {
  ...
  for (const invalid of [
    snapshot(['Game', 'game'], []),
    snapshot(['../Game'], []),
    snapshot(['Collection_Missing/Game'], []),
    snapshot(['Collection_One']),
    snapshot(['Collection_One/Nested/Game']),
    { status: 'complete', games: [] },
    { status: 'incomplete', games: [], collections: [] },
  ]) {
...
test('a mid-transaction SQL failure rolls back new entries and missing/last-seen changes', (t) => {
  ...
  assert.throws(
    () =>
      repo.reconcile(snapshot(['New Game', 'Fail'], ['Collection_New']), later),
    /fixture failure/
  );
  assert.deepEqual(
    { games: repo.listGames(), collections: repo.listCollections() },
...
test('file catalog lives outside the scan tree, survives restart and portable-base relocation', async (t) => {
  ...
  try {
    seed
      .prepare('UPDATE games SET manual_overrides = ?')
      .run(JSON.stringify({ title: 'Personal title', description: null }));
  } finally {
    seed.close();
  }
  const before = repo.listGames();
  ...
  try {
    assert.deepEqual(reopened.listGames(), before);
  } finally {
    reopened.close();
  }
});
...
test('read-only database rejects reconciliation without losing existing entries', async (t) => {
  ...
  const before = repo.listGames();
  repo.close();
  const readonly = new CatalogRepository(
    new Database(join(base, 'data', 'library.db'), { readonly: true })
  );
  try {
