...
import { TheGamesDbProvider, TheGamesDbError } from '../src/main/metadata/thegamesdb.ts';
import { ConfigService } from '../src/main/config/service.ts';
import { configuredProviders, providerSession } from '../src/main/metadata/service.ts';
import { LibraryService } from '../src/main/library/service.ts';
import type { OperationResult } from '../src/shared/api.ts';

const game = { id: 7, game_title: 'Game A', release_date: '2000-01-01', overview: 'Fixture overview' };
const json = (value: unknown, status = 200, headers: Record<string, string> = {}) => new Response(JSON.stringify(value), { status, headers });
const result = (games: unknown[], next: string | null = null) => ({ code: 200, data: { count: games.length, games }, pages: { next } });
const errorCode = (code: string) => (error: unknown) => error instanceof TheGamesDbError && error.code === code;
const value = <T>(result: OperationResult<T>): T => { if (!result.ok) assert.fail(result.message); return result.value; };

function fixture(responses: (Response | Error)[]) {
  let now = 0;
  const calls: { url: URL; init: RequestInit; at: number }[] = [];
  const provider = new TheGamesDbProvider({ enabled: true, apiKey: 'fixture-key &+#' }, {
    now: () => now, sleep: async ms => { now += ms; },
    fetch: async (url, init) => {
      calls.push({ url: new URL(String(url)), init: init!, at: now });
      const response = responses.shift();
      if (!response || response instanceof Error) throw response ?? new Error('Unexpected request');
      return response;
    }
  });
...
test('TheGamesDB encodes key/query and normalizes details with cached company/genre names', async () => {
  const detail = { ...game, developers: [1], publishers: [2], genres: [3], rating: 'E - Everyone' };
  const f = fixture([json(result([game])), json(result([detail])),
    json({ code: 200, data: { developers: { '1': { id: 1, name: 'Studio' } } } }),
    json({ code: 200, data: { publishers: { '2': { id: 2, name: 'Publisher' } } } }),
    json({ code: 200, data: { genres: { '3': { id: 3, name: 'Adventure' } } } }), json(result([detail]))]);
  assert.deepEqual(await f.provider.search('Game A & 日本語'), [{ recordId: '7', title: 'Game A', releaseYear: 2000 }]);
  const details = await f.provider.getGame('7');
  assert.deepEqual(details, { recordId: '7', title: 'Game A', releaseYear: 2000, description: 'Fixture overview',
    developers: ['Studio'], publishers: ['Publisher'], genres: ['Adventure'], artwork: [] });
  ...
  for (const call of f.calls) {
    ...
    assert.equal(call.init.redirect, 'error'); assert.equal(call.init.method, 'GET');
  }
  assert.ok(f.calls.slice(1).every((call, index) => call.at - f.calls[index].at >= 300));
});

test('TheGamesDB omits missing values, returns empty search/null details, and validates inputs', async () => {
  const f = fixture([json(result([])), json(result([])), json(result([{ id: 7, game_title: 'Undated', release_date: '0000-00-00' }]))]);
  assert.deepEqual(await f.provider.search('Absent'), []);
  assert.equal(await f.provider.getGame('7'), null);
  assert.deepEqual(await f.provider.getGame('7'), { recordId: '7', title: 'Undated', artwork: [] });
  for (const id of ['0', '01', '../7', '7&apikey=other', '9007199254740992']) await assert.rejects(f.provider.getGame(id), errorCode('invalid-record-id'));
  for (const query of ['', 'a\nb', 'x'.repeat(251)]) await assert.rejects(f.provider.search(query), errorCode('invalid-query'));
...
test('TheGamesDB rejects partial or malformed results without following response URLs', async () => {
  ...
  await assert.rejects(mismatch.provider.getGame('7'), errorCode('invalid-response'));
});

test('TheGamesDB failures expose no key, URL, response body or transport error', async () => {
  for (const response of [json({ message: 'fixture-key' }, 403), new Error('https://api.thegamesdb.net?apikey=fixture-key'),
    new Response('fixture-key'), json({ code: 403, status: 'fixture-key' }), json({}, 500)]) {
    const f = fixture([response]);
    await assert.rejects(f.provider.search('Game A'), error => {
      assert.ok(error instanceof TheGamesDbError);
      assert.ok(!String(error).includes('fixture-key')); assert.ok(!String(error).includes('https://')); return true;
    });
  }
});

test('TheGamesDB respects rate-limit and allowance cooldown without automatic retries', async () => {
  const f = fixture([json({}, 429, { 'retry-after': '5' }), json(result([game]))]);
  await assert.rejects(f.provider.search('Game A'), errorCode('rate-limited'));
  await assert.rejects(f.provider.search('Game A'), errorCode('rate-limited'));
  assert.equal(f.calls.length, 1);
  f.advance(5000);
  assert.equal((await f.provider.search('Game A')).length, 1);
  const quota = fixture([json({ ...result([game]), remaining_monthly_allowance: 0, extra_allowance: 0, allowance_refresh_timer: 60 })]);
...
test('TheGamesDB rejects disabled/unconfigured calls and oversized responses', async () => {
  for (const settings of [{ enabled: false, apiKey: 'fixture-key' }, { enabled: true, apiKey: '' }]) {
    let calls = 0;
    const provider = new TheGamesDbProvider(settings, { fetch: async () => { calls++; throw new Error(); } });
    await assert.rejects(provider.search('Game A'), errorCode(settings.enabled ? 'unconfigured' : 'disabled'));
    assert.equal(calls, 0);
  }
  const f = fixture([new Response('x'.repeat(2 * 1024 * 1024 + 1))]);
  await assert.rejects(f.provider.search('Game A'), errorCode('response-too-large'));
});

test('TheGamesDB times out stalled requests and rejects overlapping work', async () => {
  const provider = new TheGamesDbProvider({ enabled: true, apiKey: 'fixture-key' }, {
    timeoutMs: 20, sleep: async () => {}, fetch: async (_url, init) => new Promise((_resolve, reject) => {
      init!.signal!.addEventListener('abort', () => reject(new Error('fixture-key')), { once: true });
    })
  });
...
async function libraryFixture(t: { after: (fn: () => Promise<void>) => void }, igdbFailure = false) {
  ...
  globalThis.fetch = async url => {
    ...
    if (target.hostname === 'id.twitch.tv') {
      ...
      return json({ access_token: 'fixturetoken', expires_in: 3600, token_type: 'bearer' });
    }
    if (target.hostname === 'api.igdb.com') return json([{ id: 1, name: 'Game A', summary: 'IGDB description' }]);
    if (target.hostname === 'api.thegamesdb.net') return json(result([game]));
    throw new Error('Unexpected request');
  };
  const service = new LibraryService(base, config, async () => '');
  t.after(async () => { service.close(); globalThis.fetch = originalFetch; await rm(base, { recursive: true, force: true }); });
...
test('configured order selects first match and preserves manual binding after reordering/rescanning', async t => {
  ...
  await writeFile(join(f.base, 'config.ini'), f.ini.replace('igdb,thegamesdb', 'thegamesdb,igdb'));
  f.calls.length = 0;
...
test('actual adapter failure falls through to TheGamesDB and persists usable local metadata', async t => {
  ...
  assert.deepEqual(f.calls, ['id.twitch.tv', 'api.thegamesdb.net', 'api.thegamesdb.net']);
});
