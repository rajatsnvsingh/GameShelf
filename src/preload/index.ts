/** Exposes the narrow, typed GameShelf IPC API to renderer code. */
...
const api: GameShelfApi = {
  ...
  openInstallFolder: (gameId) =>
    ipcRenderer.invoke(OPEN_INSTALL_FOLDER_CHANNEL, gameId),
  openContainerFolder: (gameId) =>
    ipcRenderer.invoke(OPEN_CONTAINER_FOLDER_CHANNEL, gameId),
  ...
  searchMatches: (gameId, query, providerId) =>
    ipcRenderer.invoke(SEARCH_MATCHES_CHANNEL, gameId, query, providerId),
  selectMatch: (gameId, providerId, recordId) =>
    ipcRenderer.invoke(SELECT_MATCH_CHANNEL, gameId, providerId, recordId),
  saveOverrides: (gameId, overrides) =>
    ipcRenderer.invoke(SAVE_OVERRIDES_CHANNEL, gameId, overrides),
  pasteArtwork: (gameId, kind) =>
    ipcRenderer.invoke(PASTE_ARTWORK_CHANNEL, gameId, kind),
  ...
  fetchArtwork: (gameId, steamGridDbId) =>
    ipcRenderer.invoke(FETCH_ARTWORK_CHANNEL, gameId, steamGridDbId),
  fetchScreenshots: (gameId) =>
    ipcRenderer.invoke(FETCH_SCREENSHOTS_CHANNEL, gameId),
  fetchMissingMetadata: () =>
    ipcRenderer.invoke(FETCH_MISSING_METADATA_CHANNEL),
  confirmArtwork: (gameId) =>
    ipcRenderer.invoke(CONFIRM_ARTWORK_CHANNEL, gameId),
  getSettings: () => ipcRenderer.invoke(SETTINGS_CHANNEL),
  saveSettings: (settings) =>
    ipcRenderer.invoke(SAVE_SETTINGS_CHANNEL, settings),
