...
import { AUTO_MATCH_CHANNEL, SEARCH_MATCHES_CHANNEL, SELECT_MATCH_CHANNEL, SAVE_OVERRIDES_CHANNEL, PASTE_ARTWORK_CHANNEL, RESCRAPE_CHANNEL, SETTINGS_CHANNEL, SAVE_SETTINGS_CHANNEL, DELETE_MISSING_CHANNEL, REBUILD_CHANNEL, MATCHING_STATUS_CHANNEL, FETCH_ARTWORK_CHANNEL, CONFIRM_ARTWORK_CHANNEL } from '../shared/api';
...
const api: GameShelfApi = {
  ...
  selectMatch: (gameId, providerId, recordId) => ipcRenderer.invoke(SELECT_MATCH_CHANNEL, gameId, providerId, recordId), saveOverrides: (gameId, overrides) => ipcRenderer.invoke(SAVE_OVERRIDES_CHANNEL, gameId, overrides), pasteArtwork: (gameId, kind) => ipcRenderer.invoke(PASTE_ARTWORK_CHANNEL, gameId, kind), replaceAllRescrape: gameId => ipcRenderer.invoke(RESCRAPE_CHANNEL, gameId), fetchArtwork: (gameId, steamGridDbId) => ipcRenderer.invoke(FETCH_ARTWORK_CHANNEL, gameId, steamGridDbId), confirmArtwork: gameId => ipcRenderer.invoke(CONFIRM_ARTWORK_CHANNEL, gameId), getSettings: () => ipcRenderer.invoke(SETTINGS_CHANNEL), saveSettings: settings => ipcRenderer.invoke(SAVE_SETTINGS_CHANNEL, settings), deleteMissing: () => ipcRenderer.invoke(DELETE_MISSING_CHANNEL), rebuildCatalog: () => ipcRenderer.invoke(REBUILD_CHANNEL)
};
