/** Parses and writes the small portable INI format used for durable application settings. */
...
export function parseIni(text: string): Ini {
  ...
  for (const [index, raw] of text
    ...
    if (header) {
      if (Object.hasOwn(ini, header[1]))
        throw new Error(`Duplicate INI section at line ${index + 1}.`);
    ...
    if (value.startsWith('"')) {
      try {
        ...
        throw new Error(`Invalid quoted INI value at line ${index + 1}.`);
      }
      if (typeof value !== 'string')
        throw new Error(`Invalid INI value at line ${index + 1}.`);
    }
...
export function withDefaults(ini: Ini): Ini {
  ...
  if (
    !ini.library.collectionPrefix ||
    /[<>:"/\\|?*\x00-\x1f]/.test(ini.library.collectionPrefix)
  ) {
    throw new Error('Invalid collection prefix.');
  }
  if (!['true', 'false'].includes(ini.library.showCollectionGames))
    throw new Error('Invalid collection visibility.');
  if (!/^(0(?:\.\d+)?|1(?:\.0+)?)$/.test(ini.metadata.matchingThreshold))
    throw new Error('Invalid matching threshold.');
  if (!['true', 'false'].includes(ini.metadata.greedyMatch))
    throw new Error('Invalid greedy matching setting.');
  if (!['title', 'releaseDate'].includes(ini.view.defaultSort))
    throw new Error('Invalid default sort.');
