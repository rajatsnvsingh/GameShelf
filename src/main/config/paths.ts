/** Resolves and validates paths relative to the portable library configuration base. */
import { win32 as path } from 'node:path';

export function portableBase(
  packaged: boolean,
  appPath: string,
...
export function validateRootLayout(
  base: string,
  root: string,
  data = path.join(base, 'data'),
  allowCrossDrive = false
): void {
  if (
    !/^[a-z]:[\\/]/i.test(root) ||
    (!allowCrossDrive &&
      path.parse(base).root.toLowerCase() !==
  ...
  if (within(root, base) || within(root, data) || within(data, root)) {
    throw new Error(
      'Choose a folder separate from GameShelf’s application and data folders.'
    );
  }
}
...
export function resolveRoot(
  ...
  if (
    !storedRoot ||
    /[\x00-\x1f]/.test(storedRoot) ||
    /^[a-z]:[^\\/]/i.test(storedRoot) ||
    (!allowCrossDrive && (path.isAbsolute(storedRoot) || /:/.test(storedRoot)))
  ) {
...
export function relativeRoot(
  ...
  return (
    path.isAbsolute(relative) ? path.normalize(root) : relative
  ).replaceAll('\\', '/');
}
