import type { GameDetails, ProviderBinding, ProviderEntry, SearchCandidate } from './provider.ts';

export interface MatchAttempt {
  providerId: string;
  outcome: 'disabled' | 'unconfigured' | 'no-results' | 'low-confidence' | 'ambiguous' | 'error' | 'not-found' | 'matched';
}

export type Resolution =
  | { status: 'preserved'; binding: ProviderBinding }
  | { status: 'matched'; binding: ProviderBinding; details: GameDetails; attempts: MatchAttempt[] }
...
function validCandidate(candidate: SearchCandidate): boolean {
  return typeof candidate?.recordId === 'string' && Boolean(candidate.recordId.trim()) &&
...
export async function resolveMetadata(
  ...
  for (const { provider, enabled, configured } of providers) {
    ...
    try {
      ...
      if (!Array.isArray(candidates) || candidates.some(candidate => !validCandidate(candidate))) throw new Error('Invalid search response.');
      ...
      if (!validCandidate(details) || details.recordId !== best.candidate.recordId ||
        normalize(details.title) !== normalize(best.candidate.title) || !Array.isArray(details.artwork)) {
        throw new Error('Inconsistent detail response.');
      }
      record('matched');
      return { status: 'matched', binding: { providerId: provider.id, providerRecordId: details.recordId,
        source: 'automatic', confidence: best.score }, details, attempts };
    } catch {
