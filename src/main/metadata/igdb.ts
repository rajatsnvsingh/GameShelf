/** Adapts IGDB authentication and responses into normalized GameShelf metadata. */
import type {
  GameDetails,
  MetadataProvider,
  SearchCandidate,
...
export type { IgdbSettings } from './provider-settings.types';
import type { IgdbSettings } from './provider-settings.types';
/** Reports a safe, provider-neutral IGDB failure to the library service. */
...
function row(value: unknown): Row {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new IgdbError('invalid-response');
...
function candidate(value: unknown): SearchCandidate {
  ...
  if (typeof item.first_release_date === 'number') {
    ...
    if (Number.isFinite(date.getTime()))
      result.releaseYear = date.getUTCFullYear();
  }
  if (
    item.cover &&
    typeof item.cover === 'object' &&
    !Array.isArray(item.cover) &&
    typeof (item.cover as Row).image_id === 'string' &&
    /^[a-zA-Z0-9_-]+$/.test((item.cover as Row).image_id as string)
  )
    result.hasArtwork = true;
  const platforms = names(list(item.platforms));
...
export function normalizeIgdbGame(value: unknown): GameDetails {
  ...
  if (
    ...
    Number.isFinite(item.total_rating) &&
    item.total_rating >= 0 &&
    item.total_rating <= 100
  )
    details.rating = item.total_rating;
  ...
  if (platforms.length) details.platforms = platforms;
  ...
  const addImage = (image: Row, kind: 'cover' | 'background', size: string) => {
    if (
      ...
      /^[a-zA-Z0-9_-]+$/.test(image.image_id)
    ) {
      artwork.push({
        kind,
        url: `https://images.igdb.com/igdb/image/upload/t_${size}/${image.image_id}.jpg`,
      });
    }
  };
  if (item.cover != null) addImage(row(item.cover), 'cover', 'cover_big');
  for (const image of list(item.artworks).slice(0, 5))
    addImage(image, 'background', '1080p');
  ...
  if (Array.isArray(item.screenshots)) {
    const ids = item.screenshots.flatMap((image) =>
      image &&
      typeof image === 'object' &&
      typeof image.image_id === 'string' &&
      /^[a-zA-Z0-9_-]+$/.test(image.image_id)
        ? [image.image_id as string]
        : []
    );
...
export class IgdbProvider implements MetadataProvider {
  ...
  private async operation<T>(work: () => Promise<T>): Promise<T> {
    ...
    if (!this.settings.clientId.trim() || !this.settings.clientSecret.trim())
      throw new IgdbError('unconfigured');
    ...
    try {
      ...
      throw error instanceof IgdbError
        ? error
        : new IgdbError('request-failed');
    } finally {
      this.busy = false;
    }
  }
    ...
    try {
      ...
      try {
        while (true) {
          ...
          if (size > 2 * 1024 * 1024) {
            await reader.cancel();
            throw new IgdbError('response-too-large');
          }
  ...
  private async accessToken(): Promise<string> {
    ...
    if (
      ...
      !/^[a-zA-Z0-9]+$/.test(result.access_token) ||
      typeof result.expires_in !== 'number' ||
      !Number.isFinite(result.expires_in) ||
      result.expires_in <= 0 ||
      result.token_type !== 'bearer'
    )
      throw new IgdbError('invalid-token-response');
    this.token = result.access_token;
    this.expiresAt =
      this.runtime.now() + Math.max(0, result.expires_in - 60) * 1000;
  ...
  async search(query: string): Promise<SearchCandidate[]> {
    if (!query.trim() || query.length > 250 || /[\x00-\x1f]/.test(query))
      throw new IgdbError('invalid-query');

    return this.operation(async () => {
      const escaped = query.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      const result = await this.games(
        `search "${escaped}"; fields name,first_release_date,cover.image_id,platforms.name; limit 50;`
      );
  ...
  async getGame(recordId: string): Promise<GameDetails | null> {
    if (!/^[1-9]\d*$/.test(recordId) || !Number.isSafeInteger(Number(recordId)))
      throw new IgdbError('invalid-record-id');

    return this.operation(async () => {
      const result = await this.games(
        `fields name,first_release_date,summary,total_rating,genres.name,platforms.name,involved_companies.developer,involved_companies.publisher,involved_companies.company.name,cover.image_id,artworks.image_id,screenshots.image_id; where id = ${recordId}; limit 1;`
      );
