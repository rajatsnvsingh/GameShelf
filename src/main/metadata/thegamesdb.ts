...
export interface TheGamesDbSettings { enabled: boolean; apiKey: string }
export class TheGamesDbError extends Error {
  readonly code: string;
  constructor(code: string) { super(`TheGamesDB: ${code}.`); this.name = 'TheGamesDbError'; this.code = code; }
}
type Row = Record<string, unknown>;
function row(value: unknown): Row {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TheGamesDbError('invalid-response');
  return value as Row;
}
...
const lookups = { developers: '/v1/Developers/ByDeveloperID', publishers: '/v1/Publishers/ByPublisherID', genres: '/v1/Genres/ByGenreID' } as const;
...
export class TheGamesDbProvider implements MetadataProvider {
  ...
  constructor(settings: TheGamesDbSettings, runtime: Partial<Runtime> = {}) {
    this.settings = { ...settings }; this.runtime = { ...defaults, ...runtime };
  }
  private async operation<T>(work: () => Promise<T>): Promise<T> {
    ...
    catch (error) { throw error instanceof TheGamesDbError ? error : new TheGamesDbError('request-failed'); }
    finally { this.busy = false; }
  }
  private async request(path: string, parameters: Record<string, string>): Promise<Row> {
    if (this.runtime.now() < this.retryAt) throw new TheGamesDbError('rate-limited');
    ...
    url.search = new URLSearchParams({ ...parameters, apikey: this.settings.apiKey }).toString();
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), this.runtime.timeoutMs);
    try {
      const response = await this.runtime.fetch(url, { method: 'GET', headers: { Accept: 'application/json' }, redirect: 'error', signal: controller.signal });
      if (!response.ok) {
        ...
        throw new TheGamesDbError(response.status === 401 || response.status === 403 ? 'key-or-quota-rejected' : 'http-error');
      }
      ...
      try {
        while (true) {
          ...
          if (size > 2 * 1024 * 1024) { await reader.cancel(); throw new TheGamesDbError('response-too-large'); }
          chunks.push(chunk.value);
        }
      } finally { reader.releaseLock(); }
      let result: Row;
      try { result = row(JSON.parse(Buffer.concat(chunks).toString('utf8'))); }
      catch { throw new TheGamesDbError('invalid-response'); }
      if (result.code !== 200) throw new TheGamesDbError(result.code === 403 ? 'key-or-quota-rejected' : 'invalid-response');
      if (result.remaining_monthly_allowance === 0 && (result.extra_allowance === 0 || result.extra_allowance === undefined)) {
        const seconds = result.allowance_refresh_timer;
        this.retryAt = this.runtime.now() + (typeof seconds === 'number' && Number.isFinite(seconds) && seconds > 0 ? seconds * 1000 : 60_000);
      }
      return result;
    } catch (error) {
      if (controller.signal.aborted) throw new TheGamesDbError('timeout');
      throw error instanceof TheGamesDbError ? error : new TheGamesDbError('request-failed');
  ...
  private games(result: Row): unknown[] {
    ...
    if (!Array.isArray(data.games) || data.games.length > 50 || data.count !== data.games.length) throw new TheGamesDbError('invalid-response');
    return data.games;
  }
  ...
  private async resolveNames(kind: keyof typeof lookups, value: unknown): Promise<string[]> {
    ...
    if (!Array.isArray(value) || value.length > 50 || value.some(id => !Number.isSafeInteger(id) || id <= 0)) throw new TheGamesDbError('invalid-response');
    const ids = [...new Set<number>(value)];
    const missing = ids.filter(id => !this.names.has(`${kind}:${id}`));
    if (missing.length) {
      const data = row((await this.request(lookups[kind], { id: missing.join(',') })).data);
      const records = row(data[kind]);
      const resolved = missing.map(id => {
        const item = row(records[String(id)]);
        if (item.id !== id || typeof item.name !== 'string' || !item.name.trim()) throw new TheGamesDbError('invalid-response');
  ...
  async getGame(recordId: string): Promise<GameDetails | null> {
    if (!/^[1-9]\d*$/.test(recordId) || !Number.isSafeInteger(Number(recordId))) throw new TheGamesDbError('invalid-record-id');
    return this.operation(async () => {
      const games = this.games(await this.request('/v1/Games/ByGameID', { id: recordId, fields: 'overview,publishers,genres' }));
      ...
      if (details.recordId !== recordId) throw new TheGamesDbError('invalid-response');
      if (typeof game.overview === 'string' && game.overview.trim()) details.description = game.overview;
