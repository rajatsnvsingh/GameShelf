/** Validates an install-folder request before delegating Explorer access through IPC. */
...
export async function resolveInstallFolder(
  root: string,
  relativePath: string,
  container = false
): Promise<string> {
  ...
  for (const part of path.parts.slice(0, -1)) {
    ...
    if (!info.isDirectory() || info.isSymbolicLink())
      throw new Error('Install folder is missing or is a link.');
    if (!within(canonicalRoot, await realpath(parent)))
      throw new Error('Install folder escapes the root.');
  }
  const target = join(parent, path.parts.at(-1)!);
  const info = await lstat(target);
  if (info.isSymbolicLink()) throw new Error('Install location is a link.');
  if (info.isDirectory()) {
    if (!within(canonicalRoot, await realpath(target)))
      throw new Error('Install folder escapes the root.');
  ...
  return info.isFile() &&
    (container || path.parts.at(-1)!.toLowerCase().endsWith('.iso'))
    ? parent
    : target;
}
