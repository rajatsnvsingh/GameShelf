/** Reads shallow filesystem listings while detecting changes and refusing links. */
...
export type { ScanReader } from './directory-reader.types';
import type { ScanReader } from './directory-reader.types';
...
export async function createScanReader(root: string): Promise<ScanReader> {
  ...
  return {
    async listDirectory(requestRoot, relativeDirectory) {
      ...
      if (
        relativeDirectory &&
        catalogPath(relativeDirectory).parts.length !== 1
      )
        throw new Error('Scan depth exceeded.');
      const directory = relativeDirectory
        ? join(canonicalRoot, relativeDirectory)
        : canonicalRoot;
      const before = await lstat(directory);
      if (!before.isDirectory() || before.isSymbolicLink())
        throw new Error('Not a regular scan directory.');
      const entries = await readdir(directory, { withFileTypes: true });
      if (!sameDirectory(before, await lstat(directory)))
        throw new Error('Directory changed while scanning.');
    ...
    async verifyUnchanged() {
      if ((await realpath(root)) !== canonicalRoot)
        throw new Error('Scan root changed.');
