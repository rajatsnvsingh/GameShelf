<!-- Presents one catalog game in the library grid and reports selection. -->
<script lang="ts">
...
>
  ...
  {#if game.matchStatus === 'unmatched'}
    <span
      ...
      aria-label="Needs matching">!</span
    >
  {/if}
  ...
  <span class="card-copy">
    ...
    {:else if showMetadata}
      <span class="card-meta">
        {#if game.metadata.releaseYear}<small>{game.metadata.releaseYear}</small
          >{/if}
        {#if game.metadata.rating !== undefined}<small
            >★ {game.metadata.rating.toFixed(0)}</small
          >{/if}
...
<style>
  ...
  .card-copy {
    ...
    background: linear-gradient(transparent, rgb(10 14 20 / 0.94) 42%);
  }
  .card-copy strong {
    display: -webkit-box;
    overflow: hidden;
