...
| Metadata | Optional; multiple enabled providers with INI priority and credentials. Exact normalized titles use provider ranking; approximate auto-matches require confidence and an unambiguous lead. An opt-in greedy setting accepts the first provider-ranked candidate. An explicit Library action can fetch fields and missing assets newly supported by GameShelf for existing bindings, without changing a binding or manual work. |
