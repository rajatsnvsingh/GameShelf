...
Provider metadata and manual overrides are separate JSON objects, so an explicit JSON `null` override survives scans distinctly from an absent property. Version 1 includes binding fields solely for persistence and preservation; no matching/editing API is implemented. Artwork tables and effective title/date query columns will be added when their feature milestones need them. The repository has no renderer API or automatic startup call yet. `out/main/catalog.js` is a separately compiled main-process entry so its native dependency and repository behavior can be exercised in Electron before Phase 5 integration.
