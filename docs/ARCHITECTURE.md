...
`LibraryService` coordinates metadata with its existing operation lock and repository. Successful manual scans reconcile locally before provider work, then pass every unresolved, unbound game to the resolver. Existing bindings are preserved. Provider errors stop further enrichment for that scan and return a warning alongside the saved local catalog. Explicit per-game automatic matching remains available for an immediate retry.
